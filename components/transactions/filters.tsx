...
export function TransactionSearchAndFilters({
  ...
  const handleFilterChange = (name: keyof TransactionFilters, value: string | number | undefined) => {
    setFilters((prev) => ({
      ...prev,
      [name]: value,
    }))
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap gap-4">
        ...
        <DateRangePicker
          ...
          onChange={(date) => {
            handleFilterChange("dateFrom", date?.from ? date.from.toISOString().split("T")[0] : undefined)
            handleFilterChange("dateTo", date?.to ? date.to.toISOString().split("T")[0] : undefined)
          }}
        />
