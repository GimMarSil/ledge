    ...
    <>
      {csvData.length === 0 && (
        <div className="flex flex-col items-center justify-center gap-2 h-full min-h-[400px]">
          <p className="text-muted-foreground">Carregue o seu ficheiro CSV para importar transações</p>
          <div className="flex flex-row gap-5 mt-8">
            <div>
              ...
              <Button type="button" onClick={() => document.getElementById("csv-file")?.click()}>
                {isParsing ? "A processar..." : <Upload className="mr-2" />} Importar CSV
              </Button>
            </div>
          </div>
        ...
        <div>
          <header className="flex flex-wrap items-center justify-between gap-2 mb-8">
            <h2 className="flex flex-row gap-3 md:gap-5">
              <span className="text-3xl font-bold tracking-tight">Importar {csvData.length} itens do CSV</span>
            </h2>
            <div className="flex gap-2">
              <Button onClick={handleSave} disabled={isSaving}>
                {isSaving ? (
                  <>
                    <Loader2 className="animate-spin" /> A importar...
                  </>
                ) : (
                  <>
          ...
          {csvData.length > MAX_PREVIEW_ROWS && (
            <p className="text-muted-foreground mt-4">e mais {csvData.length - MAX_PREVIEW_ROWS} entradas...</p>
          )}
        </div>
      )}
    </>
